...
export type CalcPositionGrossPnlInfoParams = {
  ...
  positionInfo?: {
    ...
    orderVolume?: number
    /** 品种配置 */
    conf?: Pick<Order.BgaOrderPageListItem, 'conf'>['conf']
  }
 ...
 *
 * 公式：
 *   买入：(currentPrice - startPrice) × orderVolume × contractSize
 *   卖出：(startPrice - currentPrice) × orderVolume × contractSize
 */
export const calcPositionGrossPnlInfo = (
  params: CalcPositionGrossPnlInfoParams,
): CalcPnlInfoResult | undefined => {
  const { positionInfo, currentPrice } = params
 ...
 */
export const calcPositionNetPnlInfo = (
  params: CalcPositionNetPnlInfoParams,
): CalcPnlInfoResult | undefined => {
 ...
 *   percent = ratio × 100
 */
export const calcPnlYieldRateInfo = (
  params: CalcPnlYieldRateInfoParams,
