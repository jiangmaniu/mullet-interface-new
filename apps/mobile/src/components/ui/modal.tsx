    ...
    <ModalContext.Provider value={{ onClose }}>
      ...
      >
        <TouchableWithoutFeedback onPress={closeOnBackdropPress ? onClose : undefined}>
          <View className={cn('flex-1 justify-center items-center', hasBackdrop && 'bg-black/80', className)}>
...
function ModalContent({ className, children, ref, ...props }: ViewProps & { ref?: React.Ref<View> }) {
  return (
    <View
      ref={ref}
      className={cn(
        'w-[281px] shrink-0 bg-special rounded-large border border-brand-default pointer-events-box-none gap-3xl',
        className,
      )}
      {...props}
    >
...
function ModalTitle({
  ...
  ref,
  ...props
}: React.ComponentPropsWithoutRef<typeof Text> & { ref?: React.Ref<typeof Text> }) {
  return (
...
function ModalFooter({ className, ref, ...props }: ViewProps & { ref?: React.Ref<View> }) {
  return <View ref={ref} className={cn('flex-row justify-end gap-4 w-full px-2xl pb-2xl', className)} {...props} />
}
