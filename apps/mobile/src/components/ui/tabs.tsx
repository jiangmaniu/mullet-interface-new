...
const tabsListVariants = cva('items-center w-full flex flex-row', {
  variants: {
    variant: {
      solid: 'gap-medium',
      outline: 'gap-medium',
...
const tabsTriggerVariants = cva('items-center justify-center flex-row', {
  variants: {
    variant: {
      solid: 'rounded-small',
...
const tabsTriggerTextVariants = cva('font-medium leading-xs', {
  variants: {
    variant: {
      solid: '',
      outline: '',
      ...
      <>
        <View className={cn(tabsListVariants({ variant, size }), tabBarClassName)}>
          <View className="flex-row" style={{ flex: renderTabBarRight ? 1 : undefined }}>
            {routes.map((route) => {
              ...
              return (
                <Pressable
                  key={route.key}
                  onLayout={(e) =>
                    onTabLayout(route.key, { x: e.nativeEvent.layout.x, width: e.nativeEvent.layout.width })
                  }
                  onPress={() => handleIndexChange(routes.findIndex((r) => r.key === route.key))}
                  className={cn(
                    tabsTriggerVariants({ variant, size, state: isActive ? 'active' : 'inactive' }),
                    tabFlex && 'flex-1',
                  )}
                >
            ...
            {variant === 'underline' && (
              <Animated.View
                className="bg-brand-primary absolute bottom-0 left-0 h-[2px]"
                style={animatedIndicatorStyle}
              />
            )}
          </View>
          {renderTabBarRight && <View className="flex-shrink-0">{renderTabBarRight()}</View>}
        </View>
        {renderTabBarBottom?.()}
      </>
    )
...
function TabsList({
  ...
  const onTriggerLayout = React.useCallback((triggerValue: string, layout: { x: number; width: number }) => {
    setLayouts((prev) => {
  ...
  const content = (
    <View className={cn(tabsListVariants({ variant, size }), scrollable && 'w-auto', className)}>
      {children}
      {variant === 'underline' && (
        <Animated.View className="bg-brand-primary absolute bottom-0 left-0 h-[2px]" style={indicatorStyle} />
      )}
    </View>
  )
  ...
  return <TabsListContext.Provider value={{ variant, size, onTriggerLayout }}>{content}</TabsListContext.Provider>
}
...
function TabsTrigger<T>({ value, children, className, onPress }: TabsTriggerProps<T>) {
  ...
  return (
    ...
    >
      <TextClassContext.Provider value={textClassName}>{children}</TextClassContext.Provider>
    </Pressable>
  )
}
