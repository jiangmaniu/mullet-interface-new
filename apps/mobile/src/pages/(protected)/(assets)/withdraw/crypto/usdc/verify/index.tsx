...
export default function VerifyScreen() {
  ...
  return (
    <View className="flex-1">
      ...
      <View className="pt-4xl gap-4xl flex-1 px-5">
        <View className="gap-xl items-center">
          ...
          <Text className="text-paragraph-p3 text-content-4 text-center">
            <Trans>我们已向 {MOCK_EMAIL} 发送了验证码，请查收并输入</Trans>
          </Text>
        </View>

        <View className="gap-xl">
          <View className="gap-medium flex-row items-center justify-center">
            {code.map((digit, index) => (
              <TextInput
                ...
                maxLength={1}
                className="text-paragraph-p1 text-content-1 border-border-1 rounded-small bg-background-1 h-14 w-12 border text-center"
      ...
      <View className="py-3xl px-5">
        <Button block size="lg" color="primary" disabled={!isCodeComplete} onPress={handleConfirm}>
          <Text>
            <Trans>确认</Trans>
          </Text>
        </Button>
      ...
      <WithdrawStatusModal visible={showSuccessModal} status="success" onClose={handleCloseSuccessModal} />
    </View>
  )
}
