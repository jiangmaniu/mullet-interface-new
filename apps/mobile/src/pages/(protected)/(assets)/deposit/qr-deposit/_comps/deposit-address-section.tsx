...
export function DepositAddressSection() {
	...
	const { data: depositAddressInfo, isLoading, isError, refetch } = useDepositAddress(
		selectedChainId,
		tradeAccountId
	);
	...
	const selectedTokenInfo = depositAddressInfo?.supportedTokens?.find(
		(token) => token.symbol === selectedToken
	);
	const minDeposit = selectedTokenInfo?.minDeposit;
	...
	return (
		<View className="gap-xl">
			...
			<View className="gap-medium">
				...
				<View className="items-center h-[140px] justify-center">
					{isLoading && (
						<View className="absolute z-10 items-center gap-small">
				...
				{/* 地址显示 + 复制 */}
				{address && (
					<View className="flex-row items-center bg-special rounded-small p-2xl gap-2xl">
			...
			{/* 注意事项 */}
			{address && <View className="gap-xl">
				<Alert variant="warning">
					<AlertText><Trans>Mullet只支持Solana链的USDC，转入非Solana链USDC资产Mullet会进行跨链桥交易/Swap交易;</Trans></AlertText>
				</Alert>

				<View>
					<Text className="text-paragraph-p3 text-content-4">
						1. <Trans>最低充值{minDeposit} {selectedToken}，低于{minDeposit} {selectedToken}不上账;(可累计充值≥{minDeposit} {selectedToken})</Trans>
					</Text>
					<Text className="text-paragraph-p3 text-content-4">
						2. <Trans>请勿转入{chainDisplayName}网络下不支持的币种，不支持的币种充值到该地址一律销毁处理;详情可查看对应网络支持的币种</Trans>
					</Text>
					<Text className="text-paragraph-p3 text-content-4">
