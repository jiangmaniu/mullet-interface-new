...
import { useSupportedTokens } from '../../_hooks/use-supported-tokens';
...
export function TokenChainSelector() {
	...
	// 获取所有链列表（包含每个链支持的代币）
	const { data: chains, isLoading: isLoadingChains } = useSupportedChains();

	// 获取全局代币配置（用于获取代币图标）
	const { data: globalTokens } = useSupportedTokens();
	...
	const getTokenIconUrl = (symbol: string) => {
		const tokenConfig = globalTokens?.find((t) => t.symbol === symbol);
