...
import { IconifyWalletSolid, IconSpinner } from '@/components/ui/icons'
 ...
 * 显示钱包地址和余额,点击进入转账页面
 */
export function ConnectedWalletCard({ walletAddress }: ConnectedWalletCardProps) {
  const { walletInfo } = useWalletInfo()
  const selectedAccount = useSelectedDepositAccount()

  // 查询钱包余额
  const { data: balanceData } = useSolanaWalletBalance(walletAddress)
  ...
  return (
    <DepositMethodCard
      icon={walletIcon}
      title={<Text className="text-paragraph-p2 text-content-1">{formatAddress(walletAddress)}</Text>}
      subtitle={
