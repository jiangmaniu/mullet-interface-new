...
export function ConnectWalletDrawer({ visible, onClose, onConnected }: ConnectWalletDrawerProps) {
  ...
  useEffect(() => {
    if (visible && !isConnected) {
      // 保存当前路径，钱包返回后会跳转回来
      useRootStore.getState().user.auth.setAuth({ redirectTo: pathname })
      setState('connecting')
      open()
    }
  }, [visible, isConnected, open, pathname, setAuth])
  ...
  return (
    <Drawer open={visible} onOpenChange={onClose}>
      <DrawerContent className="pb-3xl gap-xl px-5">
        <View className="pt-3xl flex-row items-center justify-between">
...
function StepRow({
  ...
  return (
    <View className="gap-medium flex-row items-start">
...
function StepIndicator({ stepNumber, status }: { stepNumber: number; status: StepStatus }) {
  ...
  return (
    <View className="bg-brand-default h-5 w-5 items-center justify-center rounded-full">
