...
import { formatMin2Time, getUID, groupBy, parseJsonFields } from './index'
...
export const formatTimeStr = (time: number[]) => {
  return groupBy(time, 2)
    .map((v: any) => [`${formatMin2Time(v[0])}-${formatMin2Time(v[1])}`])
    .flat()
...
export const getBuySellInfo = (item: any) => {
  ...
  const buySellText = isBuy ? t`BUY` : t`SELL`
...
export const getDictLabelByLocale = (value: string) => {
  ...
  return i18n.locale === 'zh-TW' ? zh : en || zh
}
