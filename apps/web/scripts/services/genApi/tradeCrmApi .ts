...
export function generateTradeCrmApi({ env = 'development' }: { env?: string } = {}) {
  console.log(env)
  const url = `http://172.31.27.8:8000/trade-crm/v2/api-docs`
  ...
  const x = generateApi({
    fileName: 'gen.ts',
    // input: resolve(process.cwd(), './scripts/services/genApi/trade-crm.json'),
    url,
    apiClassName: 'TradeCrmApi',
    singleHttpClient: true,
    output: resolve(process.cwd(), './src/services/api/trade-crm/instance'),
...
export function timingGenerateTradeCrmApi({
  timing,
  env,
}: {
  /**
