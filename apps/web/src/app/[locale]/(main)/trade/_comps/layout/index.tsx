...
// 固定高度配置（组件外部常量）
const FIXED_HEIGHTS = {
  [TradeLayoutKey.Tabs]: 34 + 6, // h值
  [TradeLayoutKey.Overview]: 60 + 6,
  [TradeLayoutKey.Tradingview]: 580 + 6,
  [TradeLayoutKey.Position]: 320 + 6,
}
...
export const TradeLayout = ({ slots }: TradeLayoutProps) => {
  ...
  useEffect(() => {
    ...
    const actionHeight = orderbooksHeight + FIXED_HEIGHTS[TradeLayoutKey.Position]
      ...
      <div className={`transition-opacity duration-500 ease-in-out ${isMounted ? 'opacity-100' : 'opacity-0'}`}>
        ...
        >
          {Object.entries(slots).map(([key, content]) => (
            <div key={key} className="relative p-[3px]">
              {content}
            </div>
          ))}
        </ResponsiveGridLayout>
