...
export type SettingPositionTpSlModalProps = {
  isOpen?: boolean
  onClose?: () => void
...
export const SettingPositionTpSlModal = observer(({ ...props }: SettingPositionTpSlModalProps) => {
  const { isOpen, onClose, onConfirm, children, positionInfo } = props
  ...
  return (
    <Modal open={isOpen} onOpenChange={onClose}>
      ...
      >
        ...
        {isOpen && <SettingPositionTpSlModalContent {...props} />}
      </ModalContent>
    </Modal>
  )
})

type SettingPositionTpSlModalContentProps = SettingPositionTpSlModalProps

const SettingPositionTpSlModalContent = observer(
  ({ positionInfo, onConfirm, onClose }: SettingPositionTpSlModalContentProps) => {
    ...
    const handleClosePosition = async () => {
      ...
      if (slFlag) {
        toast.error(msg)
        setSl(positionInfo.stopLoss)
        return
      }
      if (spFlag) {
        toast.error(msg)
        setSp(positionInfo.takeProfit)
        return
      }

      const params = {
        bagOrderId: positionInfo.id,
