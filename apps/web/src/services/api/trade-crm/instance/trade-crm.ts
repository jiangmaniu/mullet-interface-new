...
import { loginInfoAtomBase } from '@/atoms/user/login-info'
import { COOKIE_LOCALE_KEY, DEFAULT_LOCALE } from '@/constants/locale'

import { HttpClient, TradeCrmApi } from './gen'

export const getTradeCrmApiInstance = () => {
  const baseUrl = `/api/trade-crm`
  ...
  const tradeCrmApi = new TradeCrmApi(httpClient)
