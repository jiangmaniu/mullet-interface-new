// @ts-nocheck
import { ChartingLibraryWidgetOptions, DatafeedConfiguration, LibrarySymbolInfo } from 'public/static/charting_library'
...
class DataFeedBase {
  ...
  resolveSymbol(symbolName, onSymbolResolvedCallback, onResolveErrorCallback, extension) {
    const currentSymbol = symbolInfoArr.find((item) => symbolName.includes(item.name))
    ...
    const commonSymbolInfo = {
      ...
      ticker: currentSymbol?.exchange // 唯一标识
  ...
  getBars(symbolInfo, resolution, periodParams, onHistoryCallback, onErrorCallback) {
    ...
    this.getDataFeedBarCallback({ symbolInfo, resolution, from, to, countBack, onHistoryCallback, onErrorCallback, firstDataRequest })
  }
